...
//  }
  ...
  should("correctly call onActiveChildChanged") {
    var appActiveChildNode: Node? = null
    var appActiveChildPath: String? = null
    var loginActiveChildNode: Node? = null
    var loginActiveChildPath: String? = null
    val sut = NavigationService(
      NavService10Schema(NavService10LoginSchema(NavService10PermissionsSchema())),
      TestNodeBuilder(
        mapOf(
          "app" to object : TestFlowNode(
            initialTarget = Target.app10.page1,
            transitions = listOf(
              tr("A", Target.app10.login { _: Int -> NavigateTo(Target.app10.page1) })
            )
          ) {
            override fun onActiveChildChanged(path: Path, child: Node) {
              appActiveChildNode = child
              appActiveChildPath = path.toString()
            }
          },
          "app.page1" to TestScreenNode(
            transitions = listOf(
          ...
          "app.page1.login" to object : TestFlowNodeWithResult<Int>(
            initialTarget = Target.login10.credentials,
            dismissResult = 33,
            transitions = listOf(
              tr(on = "A", target = Target.login10.permissions { result: String -> Finish(result.toInt()) }),
            ...
            override fun onActiveChildChanged(path: Path, child: Node) {
              loginActiveChildNode = child
              loginActiveChildPath = path.toString()
            }
          },
          "app.page1.login.credentials" to TestScreenNode(
            tag = "app.page1.login.credentials"
          ),
          "app.page1.login.credentials.permissions" to TestFlowNode(
            initialTarget = Target.permissions10.intro,
            transitions = listOf(
            ...
            tag = "app.page1.login.credentials.permissions",
          ),
          "app.page1.login.credentials.permissions.intro" to TestScreenNode(),
        )
      ),
    ...
    sut.collectTransitions().test {
      awaitItem().active shouldBe "app.page1"
      (appActiveChildNode as? HasTag)?.tag shouldBe "app.page1"
      appActiveChildPath shouldBe "app.page1"
      (loginActiveChildNode as? HasTag)?.tag shouldBe null
      loginActiveChildPath shouldBe null

      sut.sendEvent(TestEvent("A"))
      awaitItem().active shouldBe "app.page1.login.credentials"
      (appActiveChildNode as? HasTag)?.tag shouldBe "app.page1.login"
      appActiveChildPath shouldBe "app.page1.login"
      (loginActiveChildNode as? HasTag)?.tag shouldBe "app.page1.login.credentials"
      loginActiveChildPath shouldBe "login.credentials"

      sut.sendEvent(TestEvent("A"))
      awaitItem().active shouldBe "app.page1.login.credentials.permissions.intro"
      (appActiveChildNode as? HasTag)?.tag shouldBe "app.page1.login"
      appActiveChildPath shouldBe "app.page1.login"
      (loginActiveChildNode as? HasTag)?.tag shouldBe "app.page1.login.credentials.permissions"
      loginActiveChildPath shouldBe "login.credentials.permissions"
      ...
      sut.sendEvent(TestEvent("B"))
      awaitItem().active shouldBe "app.page2"
      (appActiveChildNode as? HasTag)?.tag shouldBe "app.page2"
      appActiveChildPath shouldBe "app.page2"
      (loginActiveChildNode as? HasTag)?.tag shouldBe "app.page1.login.credentials.permissions"
      loginActiveChildPath shouldBe "login.credentials.permissions"
