...
class ParallelNodeTest : ShouldSpec() {
  init {
    xshould("resolve initial state with basic parallel setup") {
      ...
      val topNodeBuilder = Par01TopNodeBuilder(
        nodeFactory = object : Par01TopNodeBuilder.Factory {
          override fun createRootNode(): FlowNode<*> = TestFlowNode(initialTarget = Target.par01Top.par01TopIntro)
      ...
      val mainNodeBuilder = Par01MainNodeBuilder(
        nodeFactory = object : Par01MainNodeBuilder.Factory {
          override fun createRootNode(): ParallelNode = TestParallelNode()
