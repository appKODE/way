...
class LoginFlowNode @Inject constructor(private val service: LoginService) : FlowNode<LoginFlowResult> {

  override val initial = Target.login.credentials
  override val dismissResult = LoginFlowResult.Dismissed

  override fun transition(event: Event): FlowTransition<LoginFlowResult> = when (event) {
